...
<html>
<head>
	<title>SSP Account Details</title>
</head>
<style type="text/css">
	body {
		font-family: Verdana;
...
<body>
	...
	<table border="1" width="900" align="center" cellpadding="6" cellspacing="0">
		<tr>
			<td width="50%">Employee Name : <?= get_name_by_auto_id('employee', get_name_by_auto_id('account_diposit', $id, 'employee_id'), 'name') ?> </td>
			<td>Printing Date : <?= date('d-m-Y') ?></td>
		</tr>
		<tr>
			<td>A/C No : <?=  $id ?>  || Account Name : <?= get_name_by_auto_id('account_diposit', $id, 'name') ?></td>
			<td></td>
		</tr>
	</table>
	...
	<table border="1" width="900" align="center" cellpadding="6" cellspacing="0">
		<tr>
			<td colspan="2"><b>SSP Collection Information : </b></td>
		</tr>
		<tr>
			<td width="200">Total Collection : </td>
			<td>
				<?php
					$total_saving = 0;
					$query = $this->db->query("select * from diposit_collection where ac_id = '$id' and sts = '1' ");
	...
	<?php
		$query = $this->db->query("select * from account_diposit where id = '$id' and sts = '1' ");
		foreach ($query->result() as $key => $value) {
	...
	<table border="1" width="900" align="center" cellpadding="6" cellspacing="0">
		<tr>
			<td colspan="6"><b>Date wise SSP Collection History: </b></td>
		</tr>
		<tr>
		    <td>Action</td>
			<td>Date</td>
		...
		$queryc = $this->db->select('*')->from('diposit_collection')->where('ac_id', $value->id)->where('sts', 1)->get();
                    foreach ($queryc->result() as $valuec) {
                     
                    ?>
		<tr>
		    <td><?php if($_SESSION['user_id'] == 0){?><a href="<?= base_url() ?>admin/delete_sspentry/<?= $valuec->Vno ?>/<?= $value->id ?>" class="btn btn-default btn-xs"  data-toggle="tooltip" title="Entry Delete" onclick="return confirm('Are you sure you want to delete this Entry?');"><i class="fa fa-remove"></i>delete</a><?php } ?></td>
