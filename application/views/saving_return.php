...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            <i class="fa fa-object-group"></i> Saving Return   
            ...
            <?php
                if (isset($_POST['f_save'])) {
                    ...
                    $data = array(
                        ...
                        'ptime'          => date("H:s:i"),
                        'Vno'            => $time.$this->input->post('ac_no', true),
                        'ctype'            => $this->input->post('c_type', true)
                    );
                    $this->db->insert('saving_collection', $data);
                    
                    $savingreturnCash = array(
                    
                          'VNo'            =>  $time.$this->input->post('ac_no', true),
                          'Vtype'          =>  'AutoSRDr',
                          'VDate'          =>  $this->input->post('pdate', true),
                          'ledger_id'      =>  1,
                          'Narration'      =>  "সঞ্চয় ফেরত হিসাব--".$_SESSION['name'],
                          'Debit'          =>  0,
                          'Credit'         =>  $this->input->post('amount_return', true),
                          'IsPosted'       =>  $this->input->post('ac_no', true),
                          'CreateBy'       =>  $_SESSION['user_id'],
                          'CreateDate'     =>  date('Y-m-d H:i:s'),
                          'IsAppove'       =>  2
                        ); 
                        $query =$this->db->insert('acc_transaction',$savingreturnCash); 
                        $savingreturn = array(
                          'VNo'            =>  $time.$this->input->post('ac_no', true),
                          'Vtype'          =>  'AutoSRDr',
                          'VDate'          =>  $this->input->post('pdate', true),
                        ...
                        $query =$this->db->insert('acc_transaction',$savingreturn);

                    if ( $this->db->affected_rows() ) {
                        $this->session->set_flashdata('msg', '<font color=green>Save Success!</font>');
                        redirect(base_url('admin/saving_return'));
                    } else {
                    ...
                    <form id="form1" action="#"  id="employeeform" name="employeeform" method="post" accept-charset="utf-8">
                        <div class="box-body">
                            ...
                            <div class="form-group">
                                <label>Date</label>
                                <input name="pdate" type="date" value="<?php echo date("Y-m-d"); ?>" class="form-control" required <?php echo ($setting['srd'] == '0') ? "readonly" : "";?> >
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="select2" name="ac_no" required style="width:100%;">
                                    ...
                                    if ($_SESSION['status']=='a') {
                                        ...
                                        $query = $this->db->select('*')->from('account')->where('employee_id', $_SESSION[user_id])->where('sts', 1)->order_by('id', 'asc')->get();
                                        foreach ($query->result() as $key => $value) {
                                            echo "<option value='$value->id'>$value->id ($value->name)</option>";
                                        }
                                    }
                            ...
                            <div class="form-group">
                                <input type="radio" id="cash_type1"
                                 name="c_type" checked="checked" value="1"><label for="contactChoice1"> Hand Cash</label>
                            
                                <input type="radio" id="cash_type2"
                                 name="c_type" value="2"><label for="contactChoice2"> Office Cash</label>
                                <span class="text-danger"></span>
                            </div>
            ...
            <div class="col-md-8">
                ...
                <div class="box box-primary">
                    <div class="box-header ptbnull">
                        <h3 class="box-title titlefix">Saving Return Summery </h3>
                    ...
                    <div class="box-body">
                        ...
                        <div class="table-responsive mailbox-messages">
                            <table class="table table-striped table-bordered table-hover example">
                                ...
                                <tbody>
                                    <?php 
                                        $query = $this->db->select('*')->from('saving_collection')->where('amount_return >', 0)->where('employee_id', $_SESSION['user_id'])->where('sts', 2)->order_by('id', 'desc')->get();
                                        foreach ($query->result() as $key => $value) {
                                    ?>
									<tr>                                               
										<td><?= ++$key ?></td>
