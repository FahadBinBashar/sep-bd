...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            <i class="fa fa-object-group"></i> Previous Collection             

            <?php
                if (isset($_POST['submit'])) {

                    if (!empty($this->input->post('installment', true))) {                  
                    ...
                    if (!empty($this->input->post('saving', true)) && $this->input->post('saving', true)>0) {                 
                    ...
                    redirect(base_url('admin/old_collection'));
                                ...
                                <tbody>
                                    ...
                                    <tr>                                               
                                        ...
                                        <?php
                                            $installment_qnt=0;
                                            $loan_id='';
                                            $queryq = $this->db->select('*')->from('account_loan')->where('ac_no', $result->ac_no)->where('sts', 1)->get();
                                    ...
                                    <form action="#" method="post" accept-charset="utf-8" enctype="multipart/form-data" name="cart">
                                        <td style="border: 1px solid #ddd;"><input type="date" name="pdate" ></td>
                                        <td style="border: 1px solid #ddd;">
                                            <?php 
                                                //today's loan collection 

                                                $querys = $this->db->select('*')->from('account_loan')->where('ac_no', $result->ac_no)->where('sts', 1)->get();
                                                ...
                                                if (isset($installment) && $installment > 0) {
                                                    ...
                                                    if (isset($keyc)) {
                                                        ...
                                                        echo "<input name='qnt' id='qnt$value->id' type='number' value='1' min='1' size='3'>";
                                                        echo "<input type='text' name='installment' id='installment$value->id' value='$value->installment_amount' placeholder='$value->installment_amount' required size='10'>";
                                            ?>
                                        ...
                                        <td style="border: 1px solid #ddd;">       
                                            ...
                                            <?php
                                                $saving_amount=0;
                                                $today = date('Y-m-d');                                                    
                                                $querys = $this->db->select('*')->from('saving_collection')->where('ac_no', $result->ac_no)->where('pdate', $today)->where('sts', 1)->get();
                                                ...
                                                if (isset($saving_amount) && $saving_amount > 0) {
                                                    ...
                                                    echo "<input type='number' name='saving' required size='10'>";
                                                }
                                            ?>                                       
